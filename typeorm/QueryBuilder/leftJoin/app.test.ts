...
describe("typeorm", () => {
  test("QueryBuilder/leftJoin", async () => {
    let selectQuery;
    ...
    expect(selectQuery).toBe(
      'SELECT 1 FROM "user" "u" LEFT JOIN "photo" "p" ON "p"."user_id"="u"."id"',
    );

    const query2 = await User.getRepository()
      .createQueryBuilder("u")
      .select("1")
      .leftJoin("u.photos", "p", "u.firstName = :name", {
        name: "qqq",
      });

    await expect(query2.getMany()).resolves.not.toThrow();
    expect(selectQuery).toBe(
      'SELECT 1 FROM "user" "u" LEFT JOIN "photo" "p" ON "p"."user_id"="u"."id" AND ("u"."firstName" = \'qqq\')',
    );
  });
});
