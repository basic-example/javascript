...
test('app', async () => {
  ...
  const qry1 = User.query().patch({ name: 'zoo', age: 1 });
  const qry2 = User.query().patch({ name: 'zoo' });
  const qry3 = User.query().patch({ name: 'zoo', age: -1 });

  expect(qry1.toKnexQuery().toString()).toBe(
    "update `users` set `name` = 'zoo', `age` = 1",
  );
  expect(qry2.toKnexQuery().toString()).toBe(
    "update `users` set `name` = 'zoo'",
  );
